...
#[1] 모든 속성의 레코드 조회 # select * from 테이블명
# * : 와일드카드
select * from member;
select* from buy;
...
select mid 회원아이디 , mname 회원명 from memeber as m;		# 속성명과 테이블명에 별칭이 가능하다.
...
#[5] 속성값에 산술연산
# 산술연산자 : 속성명 + 값 , 속성명 - 값 , 속성명 * 값 , 속성명 / 3 , 속성명 div 값 ( 몫 구하기 ) , 속성명 mod 값 (나머지 구하기)
select mnumber as 인원수 , mnumber + 3 as 더하기 , mnumber - 3 as 빼기 , mnumber * 3 as 곱셈 , mnumber / 3 as 나누기 ,
	mnumber div 3 as 몫 , mnumber mod 3 as 나머지 from member;
# as 생략 가능

#[6] 조건절 where
# 비교연산자 : 속성명 = 값; ( = 같다 ) , 속성명 != 값; ( != 같지않다 ) 
# 논리연산자 : not 조건 , 조건1 and 조건2 , 조건1 or 조건2
# 기타연산자 : 속성명 between 시작값 and 끝값 , 속성명 in (값1 , 값2 , 값3) , 속성명 is null , 속성명 is not null
# 문자열패턴 : 속성명 like '문자패턴'
			# % : 모든 문자 대응 , 문자 개수 상관없음		ex) 삼성전자/LG전자/물전자 , %전자 -> 삼성전자/LG전자/물전자
            # _ : _개수 만큼 문자 대응				ex) 삼성전자/LG전자/물전사 , _전자 -> 물전자
# 주의할 점 : 문자열 처리는 " " or ' ' 권장 , 
...
select * from member where mheight >= 165 and mheight <= 170; 	# 'mheight' 속성값이 165 이상이면서 170이하인 레코드 조회
select * from member where mheight between 165 and 170;			# 'mheight' 속성값의 165 ~ 170 사이인 레코드 조회
# 'maddr' 속성값이 '경기' or '전남' or '경남'인 레코드 조회
select * from member where maddr = '경기' or maddr = '전남' or maddr = '경남';
select * from member where maddr in ('경기' , '전남' , '경남');		# 'maddr' 속성값이 '경기' or '전남' or '경남'인 레코드 조회
select * from member where mphone1 = '';
select * from member where mphone1 = null;						# *** 속성값이 null 일 때 = (같다) 불가능
select * from member where mphone1 is null;						# 속성명 is null; 속성값이 null인 레코드 조회
select * from member where mphone1 is not null;					# 속성명 is not null; 속성값이 null이 아닌 레코드 조회
...
# order by 속성명1 정렬기준 , 속성명2 정렬기준;	, 지정한 속성명의 정렬이 2개 이상일 때 ,(쉼표)로 구분		, 2차정렬은 1차정렬내 동일한 값 내에서 2차정렬
...
select * from member order by maddr desc , mdebut asc;			# 먼저 'maddr' 속성값을 정렬 후 동일한 속성값이 존재할 경우 동일한 속성값끼리 'mdebut'을 정렬한다.
...
select * from member order by mheight desc limit 3;				# 'height'가 상위 3개만 조회 제한

# 실습 1 : 주소가 '서울'인 레코드를 데뷔일 기준으로 내림차순으로 조회 결과에 상위 2개만 조회하시오
...
# 주의할 점 : 두 개 이상의 속성명 사용 ( 그룹대상이 동일하더라도 다른 속성명의 값이 다를 수 있으므로 )
select * from buy;														# 'buy' 테이블 전체 레코드 조회
select bpname from buy group by bpname;									# 'buy' 테이블의 'bpname' 속성명 값을 그룹 , 중복 값이 없다.
# select bpname , bprice from buy group by bpname;						# 오류 발생 : bpname이 동일하더라도 bprice 다를 수 있으므로
/*
	학년		점수		1학년의 총 점수합계
    1		30
    2		20
...
select min(bamount) from buy;											# 'bamount'(구매수량) 속성명 값들의 최소값
select max(bamount) from buy;											# 'bamount'(구매수량) 속성명 값들의 최대값
select count(bamount) from buy;											# 'bamount'(구매수량) 속성명 값들의 개수 , null 제외
select count(*) from buy;												# 'bamount'(구매수량) 속성명 레코드의 개수 , null 포함

#1. 회원아이디 별 구매수량 총합계
select mid , sum(bamount) from buy group by mid;						# 'mid' 속성명의 값으로 (중복)그룹 하고 'bamount' 속성명으로 집계
#2. 회원아이디 별 총 구매금액 조회 , 구매수량 * 구매가격
...
#[11] having 그룹 후 조건(별칭사용가능) vs where 그룹 전 조건 (별칭 사용 불가) , (일반조건)
# 주의할 점 : select 문에서 컴퓨터가 처리하는 순서 : select[3] from[1] where[2] group by[4] having[5]
#1. 회원아이디별 구매수량(bamount)이 3초과하고 회원아이디별 인 레코드 조회
...
# select mid , sum(bamount * bprice) as 총구매금액 from buy where 총구매금액 >= 1000; # 오류 : 
...
# 실습 2 : 구매수량(bmount)이 2개 초과인 레코드에서 회원아이디별 구매가격(bprice) 평균이 50 이상인 레코드 조회
# + 구매가격 평균으로 내림차순 조회 , 상위 2개만 조회
# 1. buy 테이블에서 구매수량 (bamount) 이 2개 초과인 레코드
select * from buy where bamount > 2;
# 2. 회원아이디(mid)별 구매가격(bprice)평균 * ~~ 별 집계 (합계/평균/개수) 그룹 쓰자
select mid as 회원아이디 , avg(bprice) as 구매가격평균 from buy where bamount > 2 group by 회원아이디;
# 3. 구매가격(bprice) 평균이 50 이상 , 구매가격평균은 집계인지 아닌지 판단 => 집계이면 having 아니면 where
select mid as 회원아이디 , avg(bprice) as 구매가격평균 from buy where bamount > 2 group by 회원아이디 having 구매가격평균 >= 50;
# 4. 구매가격평균 내림차순 조회
