...
public class Student { // 객체를 만들기 위한 설계 클래스
	//* 클래스의 구성멤버 :  멤버 변수 , 멤버 함수 (메소드) , 생성자
	// 1. 멤버 변수				//객체가 가질 수 있는 고유한 성질/특성/값		*상태
	int studentID; // 학생번호
	...
	// 2. 메소드(멤버함수)			// 객체가 행위할 수 있는 명령어 집합/묶음		*행위
		// - 메소드 정의/만들기 : 현재 객체의 학생명을 호출했던 곳으로 반환하는 함수(출력)
		// -String(반환타입) getStudentName(함수명) , () : 매개변수생략 , {} : 실행코드 , return : 함수 종료 및 반환값
