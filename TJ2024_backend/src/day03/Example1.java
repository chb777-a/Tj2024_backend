...
public class Example1 {// 자바는 번역(컴파일)단위를 클래스 단위로 함으로써 모든 코드는 클래스안에 존재
	...
	public static void main(String[] args) {
		...
		// [단락회로평가] 앞의 항에서부터 false 이므로 , 뒤의 항(조건)은 볼 필요없으므로 코드 실행되지 않는다.
		System.out.println(value3);
		System.out.println(num1); // 20
		System.out.println(i); //4 --> 2 뒤의 항은 연산 실행되지 않음
		
		value3 = ((num1 = num1 + 10)>10 || (i = i + 2)<10);
		// 앞의 항에서부터 true 이므로 , 뒤의 항(조건)은 볼 필요없으므로 코드 실행되지 않는다.
		...
		int num3 = 5 > 3 ? 10 : 20; // 만일 5가 3보다 크면 10대입하고 아니면 20을 대입한다.
